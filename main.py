...
def build_jump_tips(agility: int = 53, barrier_high: int = 50) -> str:
    ...
    Формула использует ту же физику, что и в auto-player:
    - скорость по agility;
    - тики до высоты барьера;
    - идеальная дистанция = speed * (ticks + 2).
    """
    from game_player import ticks_to_reach_height, _agility_to_speed_per_ms

    agility = max(1, min(100, int(agility)))
    barrier_high = max(10, min(120, int(barrier_high)))

    speed_px_ms = _agility_to_speed_per_ms(agility)
    speed_px_tick = speed_px_ms * 10
    ...
    too_early = max(0, ideal_dist - speed_px_tick * 8)
